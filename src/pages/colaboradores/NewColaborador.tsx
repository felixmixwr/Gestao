...
export default function NewColaborador() {
  ...
  const {
    ...
    defaultValues: {
      nome: '',
      funcao: 'Motorista Operador de Bomba',
      tipo_contrato: 'fixo',
      salario_fixo: 0,
      registrado: false,
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      addToast({ message: 'Colaborador criado com sucesso!', type: 'success' })
      navigate('/colaboradores')
    } catch (err: any) {
      console.error('Erro ao criar colaborador:', err)
      addToast({ message: err?.message || 'Erro ao criar colaborador', type: 'error' })
    }
  }
  ...
  return (
    <Layout>
      <div className="space-y-6 max-w-5xl">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Novo Colaborador</h2>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => navigate('/colaboradores')} disabled={isSubmitting}>
              Cancelar
            </Button>
