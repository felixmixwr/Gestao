-- Script CORRIGIDO para atualizar a migração de NOTA_EMITIDA com configurações personalizadas
    ...
    IF NEW.status = 'NOTA_EMITIDA' AND (OLD.status IS NULL OR OLD.status != 'NOTA_EMITIDA') THEN
        ...
        IF NOT EXISTS (
            ...
            INSERT INTO public.pagamentos_receber (
                ...
                CURRENT_DATE + prazo_dias_val, -- Sintaxe mais simples
                prazo_dias_val,
...
BEGIN
    ...
    ELSE
        v_data_vencimento := CURRENT_DATE + p_prazo_dias; -- Sintaxe mais simples
    END IF;
