-- Migration: Add Missing Audit Triggers (Safe Version)
-- Description: Add audit triggers for tables that were missing from the initial audit system
-- This version avoids deadlocks by using smaller transactions and checking table existence
...
-- 2. Add audit triggers for missing tables (one by one to avoid deadlocks)
...
-- 3. Update the audit dashboard view to include new tables
...
-- 4. Update the system statistics view to include new tables (with safe checks)
...
-- 5. Create a function to get all monitored tables
