-- =============================================
-- AJUSTE FINAL DOS KPIs - COM VALOR AGUARDANDO
-- =============================================
-- Este script inclui o valor dos relatórios aguardando pagamento

-- 1. REMOVER VIEW EXISTENTE PARA RECRIAR COM NOVOS NOMES
DROP VIEW IF EXISTS view_kpis_financeiros_unificados;

-- 2. RECRIAR VIEW COM KPIs AJUSTADOS CONFORME SOLICITAÇÃO
CREATE VIEW view_kpis_financeiros_unificados AS
SELECT
    ...
    -- Relatórios (AJUSTADOS CONFORME SOLICITAÇÃO)
    ...
    -- Valor dos relatórios aguardando pagamento
    (SELECT COALESCE(SUM(total_value), 0) FROM reports WHERE status = 'AGUARDANDO_PAGAMENTO') as valor_relatorios_aguardando_pagamento,
    ...
    -- Métricas calculadas (AJUSTADAS CONFORME SOLICITAÇÃO)
...
SELECT '=== VIEW RECRIADA COM SUCESSO ===' as info;
...
SELECT 
    ...
    valor_relatorios_aguardando_pagamento,
...
-- 4. TESTE DETALHADO DOS NOVOS KPIs
...
COMMENT ON VIEW view_kpis_financeiros_unificados IS 'View recriada com KPIs conforme especificação: Total bombeamentos, Aguardando pagamento com valor, Relatórios pendentes';

SELECT '=== KPIs AJUSTADOS COM SUCESSO ===' as info;
SELECT 'Agora os KPIs estão conforme sua especificação com valor no Aguardando' as resultado;
