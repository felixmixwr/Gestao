-- Setup do banco de dados para notificações push - VERSÃO CORRIGIDA
-- Execute este SQL no Supabase SQL Editor

-- 1. Primeiro, adicionar colunas de notificação na tabela users
ALTER TABLE users 
...
-- 2. Criar tabela para log de notificações enviadas (se não existir)
...
-- 3. Criar índices para performance (após criar a tabela)
...
-- 4. Criar função para limpar tokens antigos
...
-- 5. Criar função para enviar notificação
...
-- 6. Criar view para estatísticas de notificações (após criar a tabela)
...
-- 11. Teste: Inserir uma notificação de teste
INSERT INTO notification_logs (user_id, notification_type, title, body, data)
VALUES (
  NULL, 
  'system', 
